...
document.addEventListener('DOMContentLoaded', ()=>{ // отправляем данные в localStorage
    const locData = JSON.parse(localStorage.getItem('data')) //берем актуальное значение
if(locData){ //если оно есть то отрисвываем
   
createCardsList(locData)
return
}
...
// пишем ф-ю для создания карточки

function createCard({name,universe,alterego,occupation,friends,superpowers,url,info,rating,id}){
    const div =document.createElement('div') //создаем див и пшем разметку
    ...
    <button class="addRaiting" id=${id}>Send</button>
...
function createCardsList(arr){ //ф-я для отрисовки карточек
...
{
    if(event.target.classList.contains('addRaiting'))//если событие по которому мы кликнули имеет класс рейтинг
    {
        saveRating(event.target.id,event.target.previousElementSibling.value)//достаем данные из инпута которые ввел пользователь и данные id
    }
})



function saveRating(id,value){// актуальный рейтинг
    const newRespomse =data.map(item => {// берем наш массив з локал и перебираем его
        if(item.id==id){ // если id совпадают
            item.rating =value // если совпадают то берем валью из инпута
            return item
        }
        return item
    })
    localStorage.setItem('data',JSON.stringify(newRespomse))// перезаписываем данные в локал 
    data=newRespomse;
}
...
//     let cardsParse = JSON.parse(cards);

//     // console.log(cardsParse);
//     let cardsContent = "";
//     for(let card of cardsParse){
//         cardsContent += `<div class='cards'>
//         <h2>${card.name}</h2>
//         <div> ВСеленная:${card.universe}</div>
//         <div> Альтер эго${card.alterego}</div>
//         <div> Род деятельности:${card.occupation}</div>
//         <div> Друзья:${card.friends}</div>
//         <div> Супер силы:${card.superpower}</div>
//         <div>${card.url}</div>
//         <div> Информация:${card.info}</div>
        
//         <div>`;
      
//     }
//     document.getElementById("cardContainer").innerHTML=cardsContent;
